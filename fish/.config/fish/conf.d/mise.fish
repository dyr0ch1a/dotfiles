...
function mise
    ...
    if contains -- --help $argv
        command /usr/sbin/mise "$command" $argv
        return $status
    end
    ...
    switch "$command"
        case deactivate shell sh
            ...
            if contains -- -h $argv
                ...
                source (command /usr/sbin/mise "$command" $argv |psub)
            end
        case '*'
            command /usr/sbin/mise "$command" $argv
    end
...
function __mise_env_eval --on-event fish_prompt --description 'Update mise environment when changing directories'
    ...
    /usr/sbin/mise hook-env -s fish | source
...
function fish_command_not_found
    if string match -qrv -- '^(?:mise$|mise-)' $argv[1] &&
            /usr/sbin/mise hook-not-found -s fish -- $argv[1]
        /usr/sbin/mise hook-env -s fish | source
